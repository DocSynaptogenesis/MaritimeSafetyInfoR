...
The data obtained in this code are available on GitHub [here](https://github.com/DocSynaptogenesis/MaritimeSafetyInfoR).
...
This is an incredibly handy `list_to_text()` function that is slightly modified from that in the [`exploratory` package](https://github.com/exploratory-io/exploratory_func).
...
total_df %>%
  ...
  filter(str_detect(relevant_terms, "MISSILE|:")) %>%
  ggplot() +
  world_map +
